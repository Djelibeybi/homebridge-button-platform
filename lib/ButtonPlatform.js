...
class ButtonPlatform extends HomebridgeLib.Platform {
  constructor(log, configJson, homebridge) {
    ...
    try {
      ...
      this.app.listen(this.config.port, () =>
        this.log(
          "Listening on port %s for inbound button push event notifications",
          this.config.port
        )
      );
  ...
  async init(beat) {
    ...
    this.app.use(
      function (req, res) {
        ...
        this.warn(
          "Received event for unconfigured Button path: %s",
          req.originalUrl
        );
      }.bind(this)
    );
  ...
  setupRoute(accessory) {
    const uri =
      "/button-" + accessory.name.toLowerCase().replace(/[^a-z0-9]/giu, "-");
      ...
      (req, res) => {
        ...
        if (!errors.isEmpty()) {
          ...
          if (req.headers["button-battery-level"]) {
            accessory.batteryService.updateBattery(
              req.headers["button-battery-level"]
            );
          }
